...
combines provider abstraction and hot-loadable modules,
...
| --- | --- | --- |
| Providers | Normalizes OpenAI-compatible APIs, Anthropic, Google, Ollama, LlamaSharp, and custom providers. | Applications keep the same agents, tools, and permissions while models change. |
| Modules | Hot-loads providers, tools, resources, gateway routes, metrics, and frontend hooks. | SharpClaw can gain new capabilities without rewriting Core. |
...
code, call a browser, use a provider, or touch a legacy system, but
...
helper model should only summarize text, a deployment workflow should need release
...
Modules are how agents get new abilities. A module can expose
...
permissions, persistence, provider routing, and audit trails.

| Want to add this feature? | Build it as this kind of module |
| --- | --- |
| Computer Use | A desktop-control module with screen capture, mouse, keyboard, policy, and approvals. |
...
| A domain workflow such as intake, release, or triage | An automation module with triggers, shared resources, logs, and optional gateway endpoints. |
...
| A capability unique to your team or industry | A purpose-built module with the tools, policies, screens, and agent contracts your users need. |

Once a module exists, the same feature can be permissioned, reused, hot-loaded,
audited, exposed to agents, and shared with other users instead of being trapped
...
| Component | Role |
| --- | --- |
| `SharpClaw.Runtime.BLL` | Agents, chat, jobs, permissions, modules, providers, costs, and logs. |
