...
/// <summary>
/// Stores Runtime event delivery records in the existing module storage
/// collection, so this boundary needs no new database table or migration.
/// </summary>
public sealed class RuntimeModuleStorageEventOutboxStore(SharpClawDbContext db)
    : IRuntimeEventOutboxStore
{
    private const string ModuleId = RuntimeEventDefinitions.ModuleId;
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var document = new OutboxDocument(
            ...
            now,
            now);
        db.ModuleStorageRecords.Add(new ModuleStorageRecordDB
        {
            Id = Guid.NewGuid(),
            ModuleId = ModuleId,
        ...
        db.ModuleStorageIndexEntries.Add(new ModuleStorageIndexEntryDB
        {
            Id = Guid.NewGuid(),
            ModuleId = ModuleId,
            StorageName = StorageName,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var keys = await db.ModuleStorageIndexEntries
            .AsNoTracking()
            .Where(index =>
        ...
        var rows = await db.ModuleStorageRecords
            .AsNoTracking()
            .Where(record =>
        ...
        CancellationToken cancellationToken)
    {
        ...
        row.ValueJson = JsonSerializer.Serialize(
            ...
            JsonOptions);
        var stateIndex = await db.ModuleStorageIndexEntries.SingleOrDefaultAsync(
            index =>
                index.ModuleId == ModuleId &&
                index.StorageName == StorageName &&
                index.IndexName == StateIndexName &&
    ...
    private async ValueTask<ModuleStorageRecordDB?> FindAsync(
        string recordKey,
        CancellationToken cancellationToken) =>
        await db.ModuleStorageRecords.SingleOrDefaultAsync(
            record =>
                record.ModuleId == ModuleId &&
                record.StorageName == StorageName &&
    ...
    private static RuntimeEventOutboxRecord Parse(ModuleStorageRecordDB row)
    {
