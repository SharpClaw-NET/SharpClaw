...
internal sealed class PackagedApplicationRegistry
{
        ...
        EndpointIngress effective)
    {
            ...
            || !string.Equals(original.Path, effective.Path, StringComparison.Ordinal))
        {
            throw new KernelActionExecutionException(
                "The Runtime request action cannot replace registration endpoint authority.");
        }
    }
    ...
    private sealed record EndpointIngress(
        string EndpointId,
        string Method,
        string Path,
        IReadOnlyDictionary<string, string[]> Headers,
    ...
    private sealed class AspNetWebSocketChannel(
        WebSocket socket,
        int maximumMessageBytes) : IWebSocketChannel
    {
        public async ValueTask<WebSocketMessage?> ReceiveAsync(
            CancellationToken cancellationToken)
        {
            ...
            do
            {
                ...
                if (result.MessageType == System.Net.WebSockets.WebSocketMessageType.Close)
                {
                    return new WebSocketMessage(
                        SharpClaw.Contracts.Kernel.WebSocketMessageType.Close,
                        [],
            ...
            return new WebSocketMessage(
                result.MessageType == System.Net.WebSockets.WebSocketMessageType.Text
                    ? SharpClaw.Contracts.Kernel.WebSocketMessageType.Text
                    : SharpClaw.Contracts.Kernel.WebSocketMessageType.Binary,
                payload.ToArray());
        }
            ...
            CancellationToken cancellationToken)
        {
            ...
            if (!message.IsWellFormed || message.Payload.Length > maximumMessageBytes)
                throw new InvalidOperationException("The registration WebSocket message is invalid.");
            if (message.Type == SharpClaw.Contracts.Kernel.WebSocketMessageType.Close)
            {
                return CloseAsync(
            ...
            return new ValueTask(socket.SendAsync(
                message.Payload,
                message.Type == SharpClaw.Contracts.Kernel.WebSocketMessageType.Text
