...
$contributionsPath = Join-Path $outputPath "contributions"
...
{
    throw "The contribution bundle output must not exist."
}

New-Item -ItemType Directory -Path $contributionsPath | Out-Null
...
$registrationIds = [System.Collections.Generic.HashSet[string]]::new([System.StringComparer]::Ordinal)
foreach ($package in $packages.Values | Sort-Object Id)
{
    ...
    try
    {
        $manifests = @(
            $zip.Entries |
                Where-Object {
                    ...
                    ($_.FullName -eq "sharpclaw/package.json" -or
                     $_.FullName -like "contentFiles/any/net10.0/contributions/*/package.json")
                }
        )
        ...
        foreach ($manifestEntry in $manifests)
        {
            ...
            if ([string]::IsNullOrWhiteSpace($SourceId) -or -not $registrationIds.Add($SourceId))
            {
                throw "The contribution bundle contains an invalid or duplicate source id."
            }

            $prefix = $manifestEntry.FullName.Substring(
                0,
                $manifestEntry.FullName.Length - "package.json".Length)
            $destination = Join-Path $contributionsPath $SourceId
            foreach ($entry in $zip.Entries |
                         Where-Object {
...
if ($registrationIds.Count -eq 0)
{
    throw "The frozen feed does not contain package manifests."
}

$manifest = [pscustomobject]@{
    ContributionCount = $registrationIds.Count
    Contributions = @(
        $registrationIds |
            Sort-Object |
            ForEach-Object {
                $registrationPath = Join-Path $contributionsPath $_
                [pscustomobject]@{
                    Id = $_
                    Files = @(
                        Get-ChildItem -LiteralPath $registrationPath -Recurse -File |
                            Sort-Object FullName |
                            ForEach-Object {
                                [pscustomobject]@{
                                    Path = [System.IO.Path]::GetRelativePath($registrationPath, $_.FullName)
                                    Length = $_.Length
...
$manifest | ConvertTo-Json -Depth 8 |
    Set-Content -LiteralPath (Join-Path $outputPath "contribution-bundle-manifest.json") -Encoding utf8

Write-Host "Contribution bundle count: $($registrationIds.Count)"
